...
class RenderCore {
    ...
    setGlobal(key, value) { this.globals[key] = value; }
            ...
            if (state[groupName]) {
                ...
                for (const ent of collection) {
                    ...
                    for (const prop of config.props) {
                        const val = ent[prop];
                        visual[prop] = (typeof val === 'number') ? Number(val.toFixed(this.rounding)) : val;
                    }
                    if (config.map) {
                        try { Object.assign(visual, config.map(ent)); } catch(e) {}
                    }
...
class UniverseCore {
    ...
    add(obj) {
        const w = obj.w || 32; const h = obj.h || 32;
...
const ActorFactory = {
    ...
    create: (name, x, y, extra = {}, state) => {
        ...
        const ent = {
            ...
            x, y, vx: 0, vy: 0, w: 32, h: 32, _type: name,
    ...
    return {
        ...
        move: (g, dt) => {
            ...
            if (Array.isArray(entities)) {
                for (let i = entities.length - 1; i >= 0; i--) if (entities[i]._dead) entities.splice(i, 1);
            }
            PhysicsCore.updateLayer(entities, layers.get(g), dt);
        },
        collide: (gA, gB, cb) => {
            if (state[gA] && layers.get(gB)) PhysicsCore.collideLayers(state[gA], layers.get(gB), cb);
        },
        find: (g) => state[g] || [],
        Actor: {
        ...
        Render: {
            ...
            config: (c) => { if(c.precision) renderSys.rounding = c.precision; }
        },
        FX: { spawn: (t,x,y) => { if(!state.effects) state.effects=[]; state.effects.push({type:t,x,y,id:Math.random()}); } },
...
function compileLogic(serverLogic, initialState, roomId, ioInstance) {
    ...
    if (!initialState._sys) Object.defineProperty(initialState, "_sys", { 
    ...
    const networkBridge = {
        sendTo: (sid, type, data) => ioInstance.to(sid).emit('custom_event', { type, data }),
        broadcast: (type, data) => ioInstance.to(roomId).emit('custom_event', { type, data })
    };
    ...
    try {
        ...
        return {
            userLogic,
            state: initialState,
            renderFn: () => roomRender.processSnapshot(initialState, initialState.effects)
        };
    } catch (e) {
...
io.on("connection", async (socket) => {
    socket.on("join_room", async (roomId, userId) => {
        ...
        if (!rooms[roomId]) {
            ...
            const { data } = await supabase.from("game_rooms").select("*").eq("name", roomId).single();
                ...
                try {
                    ...
                    if (r.state._sys && r.state._sys.spawnQueue.length > 0) {
                        ...
                        while(q.length > 0) {
                            ...
                            const ent = ActorFactory.create(type, req.config.x||0, req.config.y||0, { isBot: true, ...req.config }, r.state);
                    ...
                    if (r.logic.onBot) {
                        for (const [botId, botCtrl] of r.bots) {
                            ...
                            if(r.state.enemies) ent = r.state.enemies.find(e => e.id === botId);
                            if(!ent && r.state.players) ent = r.state.players.find(e => e.id === botId);
    ...
    socket.on("sim_input", (p) => {
        if (socket.roomId && rooms[socket.roomId]) {
            p.id = socket.userId || socket.id;
            rooms[socket.roomId].inputQueue.push(p);
        }
    });
    ...
    socket.on("voice_join", () => {
        if(socket.roomId) socket.to(socket.roomId).emit("voice_user_joined", socket.id);
    });
    socket.on("voice_signal", (p) => io.to(p.targetId).emit("voice_signal", { senderId: socket.id, signal: p.signal }));
});
